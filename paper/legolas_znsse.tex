...
\begin{itemize}[noitemsep]
    \item Lattice mismatch between \zns{} (5.41~\AA) and \znse{} (5.67~\AA)
...
\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{figures/vegard_law_bandgap.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{figures/hardware_diagram.pdf}
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Electrolyte:} \ce{I^-/I_3^-} redox couple in acetonitrile
...
\label{alg:legolas}
\begin{algorithmic}[1]
\REQUIRE Number of iterations $N$, initial samples $N_0$, exploration parameter $\xi$, candidates $M$
...
\STATE Pre-compute candidates: $\{x_1, \ldots, x_M\} \in [0,1]$ \COMMENT{Computed once}
\STATE Pre-compute bandgaps: $\{E_{g,1}, \ldots, E_{g,M}\}$ \COMMENT{Deterministic}
\FOR{$i = 1$ to $N_0$}
...
\FOR{$i = N_0 + 1$ to $N$}
    \STATE Evaluate EI: $\{\text{EI}(E_{g,1}), \ldots, \text{EI}(E_{g,M})\}$ \COMMENT{Using pre-computed $E_g$}
...
Key features:
\begin{itemize}[noitemsep]
    \item \textbf{Pre-computation (lines 3--4):} Candidate grid and bandgaps computed once (not per iteration)
    \item \textbf{Phase 1 (lines 5--12):} Random exploration builds initial GP model
    \item \textbf{Phase 2 (lines 14--24):} EI-guided optimization refines search using pre-computed grid
    \item \textbf{Model updating (line 23):} GP retrained after each measurement
    \item \textbf{Composition mapping (line 17):} Bandgap-to-composition conversion
\end{itemize}
...
\begin{figure*}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/optimization_convergence_trajectory.pdf}
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Convergence:} Optimal region identified by iteration 7 (x_{\text{Se}} < 0.1)
\end{itemize}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{figures/gp_regression_model.pdf}
...
\begin{figure*}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/composition_space_exploration.pdf}
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Open source:} Full code available at \url{https://github.com/samridhi-chordia/legolas-znsse}
...
Saar, L. (2023). LEGO-based Low-cost Autonomous Scientist (LEGOLAS). \textit{MRS Bulletin}, 48, 156--161. \url{https://link.springer.com/article/10.1557/s43577-022-00430-2}
